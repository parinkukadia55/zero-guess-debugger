...
Before marking any task complete, verify all 6 layers:
...
2. **Mandatory Pre-Fix Diagnostic Card:**
   ...
   - `[BLAST RADIUS]`: All callers/consumers, routes, endpoints, and panels audited.
...
5. **Bidirectional Reverse-Check:** Forward trace (Admin Panel -> Endpoint -> Router -> Home UI) and reverse check (failure backwards to origin).
